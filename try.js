...
var maxSubArrayIndis = 0; //biggest sum index
var array = [
  0,
  1,
  2,
...
function lastMember(index) {
  ...
  var tmpSubArray = subArray;
  var temp = (tmpSubArray.length - 1) * 2; //patlak sıkışık dizi dışında çalışmıyor
  //temp değerini indisbelirle fonksiyonunu çağırdıktan sonra arttır
  for (let index2 = temp; index2 < tmpArray.length; index2++) {
    //push gerçekleştir
    subArrayFilteredIndex.push(tmpSubArray.toString());

    console.log("maxSUM=" + maxSubArraySum);
    console.log(tmpSubArray);
    console.log("maxIndis=" + maxSubArrayIndis);

    //subArray index dizisine at
    checkSumIndex(tmpSubArray, tmpArray);
    tmpSubArray[tmpSubArray.length - 1]++; //subArray son elemanı bir arttırarak gez.
  }
  return tmpSubArray;
}
